<div class="header">
    <h1 class="page-title">Profile Text</h1>
</div>
...
<div class="search-well">
    <?php
    echo $this->Form->create('User',array('type' => 'post','url' => array('controller' => 'profiletext','action' => 'admin_profiletext','admin' => TRUE)));
...
<div style="float: left">
	...
	<script>
		$("#show").change(function(){
			$limit=$("#show").val(); 
          location.href="<?php echo ROOT_URL;?>admin/profiletext/profiletext/"+$limit;
     });
	</script>
    ...
    <table class="table">
        <thead>
            <tr>
                ...
                <th class="sort <?php if(isset($this->params['paging']['User']['order']['User.profile_text_change_date']) && ($this->params['paging']['User']['order']['User.profile_text_change_date']=='asc')){ echo 'asc';}elseif(isset($this->params['paging']['User']['order']['User.profile_text_change_date']) && $this->params['paging']['User']['order']['User.profile_text_change_date']=='desc'){echo 'desc';} ?>"><?php echo $this->Paginator->sort('User.profile_text_change_date', 'Post Date');?></th>
<!--				-->
                ...
                foreach ($results as $loop => $result) {
                    ...
                    <tr>
                       ...
                       <!-- <td><img src="<?php echo $this->webroot;?><?php echo $path; ?>" height="50" width="50"></td>-->
						...
						<td><?php echo $result['User']['profile_text_change_date']; ?></td>
						<!--<td> --><?php
...
<!--                        <td><?php if($result['User']['member_type']==1 && strtotime($result['User']['valid_upto'])<strtotime(date('Y-m-d')) || $result['User']['member_type']==0){ echo '----';}else {echo date('Y-m-d',strtotime($result['User']['valid_upto']));} ?></td>-->
                        ...
                        <td>
                           <!-- <a href="<?php echo $this->Html->url(array('action' => 'edit', 'admin' => TRUE, $result['User']['id'])); ?>" title="Edit"><i class="icon-pencil"></i></a>-->
							...
							if ($result['User']['profiletext_change'] == 0) {
                            ...
                            ?>
							<a onclick='if (confirm("Are you sure you want to <?php echo $text_status;?> User?")) { return true; } return false;' href="<?php echo $this->Html->url(array('action' => 'change_profiletext', 'admin' => TRUE, $result['User']['id'], $text_status_val)); ?>" title="Change Status to <?php echo $text_status; ?>"><i class="<?php echo $class1; ?>"></i></a> &nbsp;
