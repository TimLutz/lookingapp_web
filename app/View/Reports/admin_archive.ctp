<div class="header">
    <h1 class="page-title">Archived Reports</h1>
</div>
...
<div class="search-well">
    <?php
    echo $this->Form->create('Flag',array('type' => 'post','url' => array('controller' => 'reports','action' => 'archive','admin' => TRUE)));
...
<div class="well">
	...
	<script>
		$("#show").change(function(){
			$limit=$("#show").val(); 
          location.href="<?php echo ROOT_URL;?>admin/reports/archive/"+$limit;
     });
	</script>
    <table class="table">
        <thead>
            <tr>
                <th class="sort <?php if(isset($this->params['paging']['Flag']['order']['Flag.id']) && ($this->params['paging']['Flag']['order']['Flag.id']=='asc')){ echo 'asc';}elseif(isset($this->params['paging']['Flag']['order']['Flag.id']) && $this->params['paging']['Flag']['order']['Flag.id']=='desc'){echo 'desc';} ?>"><?php echo $this->Paginator->sort('Flag.id', 'SR #',['direction' => 'desc']);?></th>
                <!--<th>Section</th>-->
            ...
            if ($count) {
                ...
                foreach ($results as $loop => $result) {
                    ...
                    <tr>
                        ...
                        //print_r($this->params['paging']['Flag']['order']['Flag.id']);?></td>
