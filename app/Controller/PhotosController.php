...
class PhotosController extends AppController {
	...
	public function admin_change_status($id, $status) {
        ...
        $this->User->updateAll(
                array('User.status' => $status,'User.photo_change' => 0), array('User.id' => $id)
        );
	...
	public function admin_change_photo($id, $text_status) {
		//echo $id."=".$status;die;
        ...
        $this->User->updateAll(
                array('User.photo_change' => $text_status), array('User.id' => $id)
        );
	...
	public function admin_photos() {
		...
		if(!isset($this->params['pass']['0'])) {
			...
			$this->paginate['User'] = array(
				'conditions' => array('User.photo_change'=>1),
				'limit' => $limit,
				'order' => array(
					'User.id' => 'DESC'
				),
